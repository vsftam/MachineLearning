...
class KMeansSuite extends FunSuite {
  ...
  test("converge") {
    ...
    val p2 = new Point(2, 0, 0)
    val p3 = new Point(1, 1, 0)
    val p4 = new Point(2, 1, 0)
    val p5 = new Point(1, 2, 0)
    val oldMeans : GenSeq[Point] = IndexedSeq(p1, p2)
    val newMeans1 : GenSeq[Point] = IndexedSeq(p3, p4)
    val newMeans2 : GenSeq[Point] = IndexedSeq(p3, p5)

    assert( converged(1.1)(oldMeans, newMeans1) === true, s"converged($oldMeans, $newMeans1) with eta 1.1 should equal to true")
    assert( converged(1.1)(oldMeans, newMeans2) === false, s"converged($oldMeans, $newMeans2) with eta 1.1 should equal to false")
  }
  ...
  test("kMeans") {
    ...
    val p7 = new Point(0.0, 0.0, 0.0)
    val p8 = new Point(0.0, 5.5, 0.0)
    val oldMeans : GenSeq[Point] = IndexedSeq(p5, p6)
    val newMeans : GenSeq[Point] = IndexedSeq(p7, p8)
    val points : GenSeq[Point] = IndexedSeq(p1, p2, p3, p4)
    assert( converged(eta)(kMeans(points, oldMeans, eta), newMeans) === true,
      s"kMeans($points, $oldMeans, $eta) should equal to $newMeans)" )
  }
}
