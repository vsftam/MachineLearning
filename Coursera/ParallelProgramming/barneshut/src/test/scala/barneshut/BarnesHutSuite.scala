...
import FloatOps._
  ...
  test("Fork with 4 empty quadrants and 1 insert") {
    val b = new Body(123f, 18f, 26f, 0f, 0f)
    val nw = Empty(17.5f, 27.5f, 5f)
    val ne = Empty(22.5f, 27.5f, 5f)
    val sw = Empty(17.5f, 32.5f, 5f)
    val se = Empty(22.5f, 32.5f, 5f)
    val quad0 = Fork(nw, ne, sw, se)
    val quad = quad0.insert(b)
  ...
  test("Leaf.insert(b) should return a new Fork if size > minimumSize") {
    val l = new Leaf(20f, 30f, 10f, Seq.empty[Body])
    val b = new Body(123f, 18f, 26f, 0f, 0f)
    val leaf = l.insert(b)

    leaf match {
      case Fork(nw, ne, sw, se) => {
        assert(nw.mass ~= 123f, s"${nw.mass} should be 123f")
        assert(nw.massX ~= 18f, s"${nw.massX} should be 18f")
        assert(nw.massY ~= 26f, s"${nw.massY} should be 26f")
        assert(ne.total == 0, s"${ne.total} should be 0")
        assert(sw.total == 0, s"${sw.total} should be 0")
        assert(se.total == 0, s"${se.total} should be 0")
      }
      case _ =>
        assert(false, "Leaf.insert(b) should return a new Fork")
    }

    assert(leaf.mass ~= 123f, s"${leaf.mass} should be 123f")
    assert(leaf.massX ~= 18f, s"${leaf.massX} should be 18f")
    assert(leaf.massY ~= 26f, s"${leaf.massY} should be 26f")
    assert(leaf.total == 1, s"${leaf.total} should be 1")
  }
  ...
  test("SectorMatrix.combine should correctly combine two sector matrices") {
    val boundaries = new Boundaries()
    boundaries.minX = 1
    boundaries.minY = 1
    boundaries.maxX = 97
    ...
    val body2 = new Body(10, 7, 8, 0.1f, 0.1f)
    ...
    val res1 = sm3(2, 3).size == 1 && sm3(2, 3).find(_ == body1).isDefined
    val res2 = sm3(0, 0).size == 1 && sm3(0, 0).find(_ == body2).isDefined
