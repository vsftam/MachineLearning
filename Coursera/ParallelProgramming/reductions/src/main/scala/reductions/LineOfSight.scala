...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var running_max = input(0)
    output(0) = running_max
    for(i <- 1 until input.length) {
      running_max = max(input(i) / i, running_max)
      output(i) = running_max
    }
  }

  object Tree {
    def toDebugStr(t: Tree) : String = t match {
      case Leaf(from, to, max) => s"Leaf($from,$to,$max) "
      case Node(l, r) => toDebugStr(l) + " .. " + toDebugStr(r)
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var m = if(from == 0) 0 else input(from) / from
    for(i <- from + 1 until until) {
      m = max(input(i) / i, m)
    }
    // println(s"upsweepSequential $from -> $until : $m")
    m
  }
