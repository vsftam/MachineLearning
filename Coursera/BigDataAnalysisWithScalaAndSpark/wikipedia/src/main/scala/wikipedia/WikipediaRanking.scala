...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] =
    sc.textFile(WikipediaData.filePath).map(s => WikipediaData.parse(s))

  private def langInWiki(lang: String, wiki: WikipediaArticle): Boolean = wiki.text.split(" ").contains(lang)
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((acc, w) => {
      val exist = if(langInWiki(lang, w)) 1 else 0
      acc + exist
    }, (a, b) => a + b)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val pairs = for(
      wiki <- rdd;
      lang <- langs if(langInWiki(lang, wiki))
    ) yield (lang, wiki)
    pairs.groupByKey()
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.mapValues(wikis => wikis.size).collect().toList.sortWith(_._2 > _._2)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    pairs.reduceByKey(_ + _).collect.toList.sortWith(_._2 > _._2)
  }
