...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'rankLangs' should work for RDD with JavaScript elements") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java", "JavaScript")
    val rdd = sc.parallelize(List(WikipediaArticle("1", "Scala is great"),
      WikipediaArticle("2", "JavaScript is OK, but Scala is cooler. People like Scala.")))
    val ranked = rankLangs(langs, rdd)
    val res = ranked.head._1 === "Scala"
    assert(res)
    val res1 = ranked.head._2 === 3
    assert(res1)
    val res2 = ranked.tail.head._1 === "JavaScript"
    assert(res2)
    val res21 = ranked.tail.head._2 === 1
    assert(res21)
    val res3 = ranked.tail.tail.head._1 === "Java"
    assert(res3)
    val res31 = ranked.tail.tail.head._2 === 0
    assert(res31)
  }

  test("'rankLangs' should work for RDD with both Java and JavaScript elements") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java", "JavaScript")
    val rdd = sc.parallelize(List(WikipediaArticle("1", "Scala is great"),
      WikipediaArticle("2", "JavaScript and Java are OK, but Scala is cooler"),
      WikipediaArticle("3","Scala and Java run on the JVM")
    ))
    val ranked = rankLangs(langs, rdd)
    println("--- ranked:" + ranked)
    val res = ranked.head._1 === "Scala"
    assert(res)
    val res2 = ranked.tail.head._1 === "Java"
    assert(res2)
    val res21 = ranked.tail.head._2 === 2
    assert(res21)
    val res3 = ranked.tail.tail.head._1 === "JavaScript"
    assert(res3)
    val res31 = ranked.tail.tail.head._2 === 1
    assert(res31)
  }
